...
class Network(object):
    ...
    def model_network(self, train_X, train_Y, test_X, test_Y):
        ...
        #testY_inverse_sent = testY_inverse_sent.reshape(-1,1)
    ...
    def remodel1(self, price_file, previous_sent, live_price, live_sentiment, predictions_file):
        ...
        print("Price Tail", price_tail)
        print("Last 4 Price", last_4_price)
        print("Sentiment Tail", sentiment_tail)
        print("Last 4 Sent", last_4_sent)


        price_tail = pd.concat([last_4_price, price_tail], axis=0)
        sentiment_tail = pd.concat([last_4_sent, sentiment_tail], axis=0)

        print("Cat price tail", price_tail)
        print("Cat sent tail", sentiment_tail)
        ...
        print("Shaped price",price)
        print("Shaped Sent", sentiment)

        price_scale = self.scale.fit_transform(price)

        print("Price Scale", price_scale)
    ...
    def remodel2(self, price_file, previous_sent, live_price, live_sentiment, predictions_file):
        ...
        #testY = testY.reshape(-1,1)
        self.testY_cont = np.concatenate((self.testY_cont, testY_inverse), axis=1)
        self.yhat_cont = np.concatenate((self.yhat_cont, yhat_inverse), axis=1)
